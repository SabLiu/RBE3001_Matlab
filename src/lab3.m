...
%        pause(2); 
%        w = fwkin3001(returnPacket(1)*angleConversion, returnPacket(4)*angleConversion, returnPacket(7)*angleConversion); 
%        printM = zeros(1,3);
%        printM(1) = w(1);
%        printM(2) = w(2);
%        printM(3) = w(3); 
...
%% Quintic Trajectory Generation for Extra Credit
%     angleConversion = (2*pi)/4095; % encoder to theta
...
% 
%   
%     vertices = [144 144 189 225 224 144;
%         133 133 108 25 -43 133;
%         -1 -1 105 170 97 -1];
%     waypoints = zeros(3,1);
%     
